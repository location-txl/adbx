...
- **TUI 文件浏览**：键盘浏览设备文件系统，跨目录多选、批量 `adb pull`，带实时筛选、排序、进度和右侧纯文本/图片预览
...
需要 Rust 1.85+（edition 2024）与 [adb](https://developer.android.com/tools/adb)（须在 PATH 中）。
...
| `browse [path] [-o <dir>]` | TUI 浏览设备文件系统，多选批量拉取到本地并预览文件 | `ls` / `pull` / `exec-out` |
...
拉取采用逐项阻塞 + 轮询本地落盘字节的方式回报近似进度；单项失败不中断整体，拉取中可随时取消，未完成项保留标记可重试。
