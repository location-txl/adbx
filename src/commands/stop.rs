//! `adbx stop` 子命令：按关键词模糊匹配包名并强制停止应用。
...
///
/// ```
/// use adbx::commands::stop::{PackageMatch, match_packages};
...
///
/// 唯一/精确命中时执行 `am force-stop` 并打印结果；
...
pub fn run(serial: Option<&str>, keywords: &[String]) -> Result<()> {
    ...
    match match_packages(&packages, keywords) {
        ...
        PackageMatch::Multiple(candidates) => Err(anyhow!(
            "多个包匹配 {}：\n{}\n  请使用更精确的关键词重试",
...
fn keywords_summary(keywords: &[String]) -> String {
    keywords
        .iter()
        .map(|k| format!("\"{k}\""))
        .collect::<Vec<_>>()
